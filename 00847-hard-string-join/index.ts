...
type getTail<T extends unknown[]> = T extends [infer Head, ...infer Tail] ? Tail : []; 
type solution<D extends string, P extends string[]> = P extends [] ? '' : P extends [infer Head] ? Head : `${P[0]}${D}${solution<D, getTail<P>>}`;
declare function join<D extends string>(delimiter: D): <P extends string[]>(...parts: P) => solution<D, P>
