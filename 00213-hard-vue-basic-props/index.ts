...
type computedType<T> = {
  [t in keyof T] : T[t] extends (...args: any) => any ? ReturnType<T[t]> : never
}

type checkType<T extends new (...args: unknown[]) => unknown> = T extends typeof String | typeof Number | typeof Boolean ? ReturnType<T> : InstanceType<T>

type propsType<T> = {
  [t in keyof T] : {} extends T[t] ? any :
  T[t] extends { type: any } ? 
    T[t]['type'] extends any[] ? checkType<T[t]['type'][number]> : checkType<T[t]['type']>
  : T[t] extends new (...args: any[]) => any ? InstanceType<T[t]> : never
}
...
type Options<P, D, C, M> = {
  ...
  methods: M & ThisType<propsType<P> & D & computedType<C> & M>
}
