...
type ParsePrintFormat<T extends string, C extends unknown[] = []> = T extends `${infer _}%%${infer A}` ? 
  ParsePrintFormat<A, C> : 
  T extends `${infer _}%${infer B}${infer D}` ? 
    B extends keyof ControlsMap ? ParsePrintFormat<D, [...C, ControlsMap[B]]> : ParsePrintFormat<D, C>
: C
