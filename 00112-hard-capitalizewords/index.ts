...
type r<T extends string, C extends string = ''> = T extends `${infer L}${infer R}` ? Uppercase<L> extends Lowercase<L> ? r<Capitalize<R>, `${C}${L}`> : r<R, `${C}${L}`> : `${C}${T}`;
type CapitalizeWords<S extends string> =r<Capitalize<S>>;
