...
type checkPercentage<T> = T extends `${infer N}%` ? [N, '%'] : [T, '']; 
type PercentageParser<A extends string> = A extends `${checkSign<infer L>}${infer R}`
  ? [checkSign<L>, ...checkPercentage<R>] : ['', ...checkPercentage<A>];
